...
public class TicTacToeController {
    private static final char[] player = {'X', 'O', 'E', 'W', 'M' , 'Y', 'F', 'Q', 'I', 'U'};
    ...
    public void processButtonClick(int row, int col)
    {
        ...
        screen.setMessage("It is " + player[(turnCount + 1) % numPlayers] + "'s turn");
        if (!curGame.checkSpace(BP)) {
            screen.setMessage("Invalid move! Player " + player[(turnCount) % numPlayers] + " it is still your turn!");
            return;
        }
        curGame.placeMarker(BP, player[turnCount %numPlayers]);
        screen.setMarker(row, col, player[turnCount % numPlayers]);
        if (curGame.checkForWinner(BP)) {
            screen.setMessage("player " + player[turnCount% numPlayers] + " wins!");
            restart = true;
        }
        else if (curGame.checkForDraw()) {
            screen.setMessage("The game is a draw!");
            restart = true;
        }
        turnCount++;
